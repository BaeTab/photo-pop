...
export default function PhotoBooth() {
    ...
    const [adClicked, setAdClicked] = useState(false); // Track ad click
    ...
    const handleStartCapture = useCallback(() => {
        if (!adClicked) {
            alert("사진을 찍으려면 상단의 광고를 한번 클릭해주세요!");
            return;
        }
    ...
    const handleDownload = async () => {
        ...
        if (!adClicked) {
            alert("다운로드하려면 상단의 광고를 한번 클릭해주세요!");
            return;
        }
    ...
    return (
        <div className="min-h-screen bg-pop-yellow p-4 md:p-8 flex flex-col items-center gap-12 font-pop overflow-x-hidden">
            ...
            {/* Keeping InfoSection here as well for users who deep link, or remove if desired. 
                For AdSense, it's better to have it on the Landing Page. 
                I will remove rendering InfoSection from here to reduce duplicate content issues if avoiding duplicate ID issues.
                But usually it's fine. let's keep it minimal for the tool.
            */}
        </div>
    );
}
